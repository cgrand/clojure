              ...
              ~@(map (fn [[x-is [x-= [x-for binds body] value]]]
                       (when (and (= x-is 'is) (= x-= '=) (= x-for 'for))
                         `(is (= (let [acc# (atom [])]
                                 ...
                                 ~value))))
                     ises))
     (deftest ~(symbol (str "RFor-" txt))
			        ~@(map (fn [[x-is [x-= [x-for binds body] value]]]
			                        (when (and (= x-is 'is) (= x-= '=) (= x-for 'for))
			                          `(is (= (into [] (r/for ~binds ~body))
			                                  ~value))))
			                      ises))
     (deftest ~(symbol (str "RDoseq-" txt))
              ~@(map (fn [[x-is [x-= [x-for binds body] value]]]
                       (when (and (= x-is 'is) (= x-= '=) (= x-for 'for))
                         `(is (= (let [acc# (atom [])]
                                   (r/doseq ~binds (swap! acc# conj ~body))
                                   @acc#)
                                 ~value))))
                     ises))))
...
(deftest-all While-and-When-Same-Binding
